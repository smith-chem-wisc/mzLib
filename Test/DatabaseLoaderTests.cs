...
namespace Test
{
    ...
    public class DatabaseLoaderTests
    {
        ...
        public void modification_read_write_into_proteinDb()
        {
            ...
            Protein protein = new Protein("MCSSSSSSSSSS", "accession", new Dictionary<int, List<Modification>> { { 2, sampleModList.OfType<Modification>().ToList() } }, null, null, null, "name", "full_name", false, false, new List<DatabaseReference>() );
            ...
            List<Protein> new_proteins = ProteinDbLoader.LoadProteinXML(Path.Combine(TestContext.CurrentContext.TestDirectory, "test_modifications_with_proteins.xml"), false, new List<Modification>(), false, null, out um);
