...
Decon TestingMainDeconvolution(Config config, Input inp, int silent, int verbose) {
    ...
    int sizeBarr = config.lengthmz * config.numz; 
    barr = calloc(sizeBarr, sizeof(char));
    // input needs modifications to make sure that it is 
    // correctly passing values to  
    ...
    if (config.mzsig != 0) {
        ...
        if (config.mzsig < 0 || config.beta < 0) {
            makereverse = 1; rmzdist = calloc(pslen, sizeof(float));
