...
namespace MassSpectrometry
{
    public static class WindowModeHelper
    {
        ...
        public static void Run(ref int[] intensities, ref double[] mArray,
           IFilteringParams filteringParams, double scanRangeMinMz, double scanRangeMaxMz, bool keepZeroPeaks = false)
        {
            Array.Sort(intensities, mArray);
            int TIC = intensities.Sum();
            ...
            Dictionary<int, List<int>> mzInRange = new Dictionary<int, List<int>>(); //index of range and  list of index values in mArray
            Dictionary<int, List<int>> mzRangeIntensities = new Dictionary<int, List<int>>(); //index of range and  list of index values in mArray
            for (int i = 0; i < ranges.Count; i++)
            {
                mzInRange.Add(i, new List<int>());
                mzRangeIntensities.Add(i, new List<int>());
            }
            ...
            List<double> reducedMzList = new List<double>();
            List<int> reducedIntensityList = new List<int>();
            ...
            foreach (int rangeIndex in mzInRange.Keys)
            {
                ...
                if (filteringParams.NormalizePeaksAcrossAllWindows)
                {
                    ...
                    int max = mzRangeIntensities[rangeIndex].Max();
                    if (max == 0)
                    {
                        max = 1;
                    }
                    mzRangeIntensities[rangeIndex] = mzRangeIntensities[rangeIndex].Select(x => 50 * x / max ).ToList();
                    // NOTE!!! This select statement used to be: x => x / max * 50.0000. If I've pemdas'd wrong, this could be a problem.
