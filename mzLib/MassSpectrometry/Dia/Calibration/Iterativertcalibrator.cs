...
namespace MassSpectrometry.Dia
{
    ...
    public class IterativeRtCalibrator
    {
        ...
        /// <summary>
        /// Maximum allowed CoElutionStd for a result to be used as a calibration anchor.
        /// Results with CoElutionStd above this threshold have scattered fragment peaks
        /// and likely represent interference rather than true targets.
        /// Units: scan count. Default 2.0 scans (~0.1 min at typical DIA acquisition rate).
        /// Set to float.MaxValue to disable this filter.
        /// </summary>
        public float AnchorMaxCoElutionStd { get; set; } = 2.0f;

        /// <summary>
        /// Minimum CandidateScoreGap for a result to be used as a calibration anchor.
        /// Results with a gap below this threshold have ambiguous peak group selection
        /// (two competing peaks of similar quality) and are unreliable RT anchors.
        /// Range [0, 1]. Default 0.05f. Set to 0f to disable this filter.
        /// </summary>
        public float AnchorMinCandidateScoreGap { get; set; } = 0.05f;

        /// <summary>
        /// Optional progress reporter. When set, calibration status messages are
        /// forwarded here in addition to Console.WriteLine. Used by the GUI/log.
        /// </summary>
            ...
            DiaExtractionOrchestrator orchestrator)
        {
...

            {
                ...
                Console.WriteLine($"  [Calibration] Bootstrap: {anchors.Count} anchors (threshold={InitialApexScoreThreshold:F2}, CoElutFilt={coElutFilt0}, GapFilt={gapFilt0})");
                ...
                int startIteration)
        {
            for (int iteration = startIteration; iteration < MaxIterations; iteration++)
            {
                ...
                Console.WriteLine($"  [Calibration] Iteration {iteration}: {anchors.Count} anchors (threshold={RefinedApexScoreThreshold:F2}, CoElutFilt={coElutFilt}, GapFilt={gapFilt})");
        ...
        /// </summary>
        /// <summary>Overload without filter diagnostics — discards filtered counts.</summary>
        public static List<(double LibraryRt, double ObservedRt, double QualityScore)> SelectAnchors(
            ...
            float maxCoElutionStd = float.MaxValue,
            float minCandidateScoreGap = 0f)
        {
            return SelectAnchors(results, precursors, groups, minApexScore, maxAnchors,
                requireMinFragDetRate, maxCoElutionStd, minCandidateScoreGap,
                out _, out _);
        }
        ...
        public static List<(double LibraryRt, double ObservedRt, double QualityScore)> SelectAnchors(
            ...
            float minApexScore,
            int maxAnchors,
            bool requireMinFragDetRate,
            float maxCoElutionStd,
            float minCandidateScoreGap,
            out int coElutionFiltered,
            out int gapFiltered)
        {
            coElutionFiltered = 0;
            gapFiltered = 0;
            ...
            for (int i = 0; i < results.Count; i++)
            {
                ...
                // NEW: co-elution quality filter — reject scattered fragment apices
                if (r.CoElutionStd > maxCoElutionStd)
                {
                    coElutionFiltered++;
                    continue;
                }

                // NEW: peak group ambiguity filter — reject ambiguous best-candidate selection
                if (r.CandidateScoreGap < minCandidateScoreGap)
                {
                    gapFiltered++;
                    continue;
                }
        ...
        /// <summary>Legacy overload without groups or quality filters.</summary>
        public static List<(double LibraryRt, double ObservedRt, double QualityScore)> SelectAnchors(
            List<DiaSearchResult> results,
            IList<LibraryPrecursorInput> precursors,
            float minApexScore,
            ...
            int gapFiltered = 0)
        {
            return new CalibrationIterationLog
            {
                ...
                CoElutionFiltered = coElutionFiltered,
                GapFiltered = gapFiltered
            };
        }
    }
    ...
    public class CalibrationIterationLog
    {
        ...
        /// <summary>Results rejected by the CoElutionStd filter during anchor selection.</summary>
        public int CoElutionFiltered { get; set; }

        /// <summary>Results rejected by the CandidateScoreGap filter during anchor selection.</summary>
        public int GapFiltered { get; set; }
        ...
        public override string ToString()
        {
            return $"Iter {Iteration}: {AnchorCount} anchors, slope={Slope:F4}, σ={SigmaMinutes:F3} min, " +
                   ...
                   $"CoElutFilt={CoElutionFiltered}, GapFilt={GapFiltered}";
        }
    }
}