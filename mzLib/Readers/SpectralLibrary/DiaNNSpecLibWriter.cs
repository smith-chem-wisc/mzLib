...
namespace Readers.SpectralLibrary.DiaNNSpectralLibrary
{
	...
	public static class DiaNNSpecLibWriter
	{
		...
		{
			// ALL markers must be written as raw bytes. The marker bit patterns are
			// not round-trippable through float literals: e.g. the Internal marker
			// 07 00 3C 40 corresponds to 2.9375016689300537f, not 2.9375f. Using
			// approximate float literals writes the wrong bytes and the reader's
			// scanner (which checks for 0x07 in byte 0) will never match them.
			byte[] bytes = t switch
			{
				DiaNNBinaryStructs.TerminusType.Internal => new byte[] { 0x07, 0x00, 0x3C, 0x40 },
				DiaNNBinaryStructs.TerminusType.NTerminal => new byte[] { 0x07, 0x00, 0x39, 0x40 },
				DiaNNBinaryStructs.TerminusType.CTerminal => new byte[] { 0x07, 0x00, 0x36, 0x40 },
				DiaNNBinaryStructs.TerminusType.NAndCTerminal => new byte[] { 0x07, 0x00, 0x33, 0x40 },
				DiaNNBinaryStructs.TerminusType.SharedConflict => new byte[] { 0x07, 0x00, 0x3D, 0x00 },
				DiaNNBinaryStructs.TerminusType.SharedSameType => new byte[] { 0x07, 0x00, 0x39, 0x00 },
				_ => new byte[] { 0x07, 0x00, 0x3C, 0x40 },
			};
			w.Write(bytes);
		}
