...
namespace Readers
{
    public class Mzml : MsDataFile
    {
                            ...
                            switch (xmlReader.Name.ToUpper())
                            {
                                ...
                                case "SPECTRUM":
                                    if (!xmlReader.IsStartElement())
                                    {
                                        ...
                                        // Construct the MzSpectrum WITH chargeArray attached (Option C: charge data
                                        // lives on the spectrum so MzSpectrum mutators keep all three arrays aligned).
                                        spectrum = new MzSpectrum(mzs, intensities, chargeArray, false);
        ...
        private static MsDataScan GetMsDataOneBasedScanFromConnection(Generated.mzMLType _mzMLConnection, int oneBasedIndex, IFilteringParams filterParams)
        {
            ...
            // so a length-only check wouldn't catch it). When the captured chargeArray length
            // doesn't match the peak count, drop it — preserving a misaligned array would
            ...
            // Construct the MzSpectrum WITH chargeArray attached (Option C: charge data
            // lives on the spectrum so MzSpectrum mutators keep all three arrays aligned).
            var mzmlMzSpectrum = new MzSpectrum(masses, intensities, chargeArray, false);
            ...
            if (msOrder.Value == 1)
            {
                return new MsDataScan(
                    ...
                    null,
                    nativeId,
                    // chargeArray no longer passed here — it lives on mzmlMzSpectrum and
                    // is exposed via MsDataScan.ChargeArray (delegating property).
                    compensationVoltage: compensationVoltage);
            }
        ...
        /// that operate on raw arrays BEFORE the MzSpectrum is constructed. (Once an
        /// MzSpectrum exists, its own mutators keep charges in lockstep automatically.)
        /// </summary>
