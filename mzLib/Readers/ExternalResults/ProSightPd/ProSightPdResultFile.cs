...
public class ProSightPdResultFile : ResultFile<ProSightPdPsmRecord>, IResultFile
{
    ...
    public ProSightPdResultFile(string filePath)
    {
        FilePath = filePath;
        Software = Software.Unspecified;
    }

    public ProSightPdResultFile() : base() { }
    ...
    public List<ProSightPdProteinRecord> LoadProteins()
    {
        using var connection = new SQLiteConnection($"Data Source={FilePath};Version=3;");
        connection.Open();

        using var command = new SQLiteCommand("SELECT * FROM TargetProteins;", connection);
        using var reader = command.ExecuteReader();

        var results = new List<ProSightPdProteinRecord>();
        while (reader.Read())
        {
            results.Add(new ProSightPdProteinRecord
            {
                Id = reader.GetInt32(0),
                Accession = reader.IsDBNull(1) ? null : reader.GetString(1),
                IsoformCount = reader.GetInt32(2),
                IsoformsWithCharacterizedProteoformsCount = reader.GetInt32(3),
                ProteoformCount = reader.GetInt32(4),
                ProteoformCharacterizationConfidence = reader.GetInt32(5),
                CharacterizedProteoformsCount = reader.GetInt32(6),
                PrSMCount = reader.GetInt32(7),
                ExcludedBy = reader.GetInt32(8),
                Description = reader.IsDBNull(9) ? null : reader.GetString(9),
                Qvalue = reader.GetDouble(10),
            });
        }
