...
namespace UsefulProteomicsDatabases
{
    ...
    public static class PrideArchiveExtensions
    {
        ...
        /// (<c>ftp.pride.ebi.ac.uk</c>) serves the identical path over HTTPS, so an FTP location is
        /// scheme-upgraded from <c>ftp://</c> to <c>https://</c>. Preference order: an explicit HTTPS location,
        /// then the FTP location upgraded to HTTPS. An Aspera-only file cannot be reached over HTTPS.
        /// </summary>
        ...
        public static bool TryGetHttpsDownloadUrl(this PrideArchiveFile file, out string httpsUrl)
        {
            ...
            // 2) Otherwise upgrade the FTP location to HTTPS (same authority + path, served over HTTPS).
            CvParam ftp = locations.FirstOrDefault(l => l?.Accession == FtpLocationAccession)
                          ?? locations.FirstOrDefault(l => l?.Value != null
                                && l.Value.StartsWith("ftp://", StringComparison.OrdinalIgnoreCase));
            if (ftp?.Value != null && ftp.Value.StartsWith("ftp://", StringComparison.OrdinalIgnoreCase))
            {
                httpsUrl = "https://" + ftp.Value.Substring("ftp://".Length);
                return true;
            }
