...
public static partial class OutlierRejection
{
    ...
    /// <param name="parameters">which outlier rejection method to use</param>
    public static void RejectOutliers(PeakBin peakBin, SpectralAveragingParameters parameters)
    {
        switch (parameters.OutlierRejectionType)
        {
            ...
            case OutlierRejectionType.PercentileClipping:
                PercentileClipping(peakBin, parameters.Percentile);
                break;

            case OutlierRejectionType.SigmaClipping:
                SigmaClipping(peakBin, parameters.MinSigmaValue, parameters.MaxSigmaValue);
                break;

            case OutlierRejectionType.WinsorizedSigmaClipping:
                WinsorizedSigmaClipping(peakBin, parameters.MinSigmaValue, parameters.MaxSigmaValue);
                break;

            case OutlierRejectionType.AveragedSigmaClipping:
                AveragedSigmaClipping(peakBin, parameters.MinSigmaValue, parameters.MaxSigmaValue);
                break;
    ...
    /// <remarks>This method is optimized by using Parallel.ForEach. This is the correct choice because we are iterating over many
    /// pixel stacks in the spectra, exceeding my general criteria for surpassing the additional overhead (roughly 1000 operations) that parallelization requires. </remarks>
    /// <param name="pixelStacktra">spectra whose outlier should be rejected</param>
    /// <param name="parameters">which outlier rejection method to use</param>
    public static void RejectOutliers(PixelStack pixelStack, SpectralAveragingParameters parameters)
    {
        Parallel.ForEach(pixelStack.PeakBins, pixelStack =>
        {
            RejectOutliers(pixelStack, parameters);
        });
    }
