...
namespace Test.Omics
{
    ...
    public class BioPolymerGroupSequenceCoverageTests
    {
        ...
        private class CoverageSpectralMatch : ISpectralMatch, IHasSequenceCoverageFromFragments
        {
            ...
            public CoverageSpectralMatch(string filePath, string baseSequence, double score, int scanNumber,
            ...
            public void GetSequenceCoverage()
            {
                ...
                var nTerm = NTerminalFragmentPositions ?? new List<int>();
                var cTerm = CTerminalFragmentPositions ?? new List<int>();
                if (!nTerm.Any() && !cTerm.Any()) return;

                var covered = new HashSet<int>();
                foreach (var pos in nTerm.Where(p => p >= 1 && p <= BaseSequence.Length))
                    covered.Add(pos);
                foreach (var pos in cTerm.Where(p => p >= 1 && p <= BaseSequence.Length))
                    covered.Add(BaseSequence.Length - pos + 1);

                FragmentCoveragePositionInPeptide = covered;
            }

            public int CompareTo(ISpectralMatch? other) => other is null ? 1 : Score.CompareTo(other.Score);
            public int CompareTo(IHasSequenceCoverageFromFragments? other) => other is ISpectralMatch sm ? CompareTo(sm) : 0;
        }
