...
namespace Test.MassSpectrometryTests
{
    public class TestXic
    {
        ...
        public static void TestXicConstruction()
        {
            ...
            NUnit.Framework.Assert.That(xic.ApexRT, Is.EqualTo(1.6f));
            NUnit.Framework.Assert.That(xic.ApexScanIndex, Is.EqualTo(6));
            NUnit.Framework.Assert.That(xic.StartRT, Is.EqualTo(1.0f));
            NUnit.Framework.Assert.That(xic.EndRT, Is.EqualTo(1.9f));
            var mass = Dist.Masses.First().ToMz(1);
            NUnit.Framework.Assert.That(xic.AveragedMassOrMz, Is.EqualTo(Dist.Masses.First().ToMz(1)).Within(0.0001));
            NUnit.Framework.Assert.That(xic.ApexPeak.Intensity, Is.EqualTo(Dist.Intensities.First() * 1e6 * 10).Within(1));

            //Test normalized peak intensities
            xic.SetNormalizedPeakIntensities();
            NUnit.Framework.Assert.That(xic.NormalizedPeakIntensities.Length, Is.EqualTo(10));
            NUnit.Framework.Assert.That(xic.NormalizedPeakIntensities.Sum(), Is.EqualTo(100f).Within(0.0001));
        }
        ...
        public static void TestXicSpline()
        {
            ...
            NUnit.Framework.Assert.That(xic.XYData.Length, Is.EqualTo(19)); 
            linearSpline.SetXicSplineXYData(xic);  
            NUnit.Framework.Assert.That(xic.XYData.Length, Is.EqualTo(19));
            //in scan cycle
            cubicSpline = new XicCubicSpline(scanIndexBased: true);
            cubicSpline.SetXicSplineXYData(xic);
            NUnit.Framework.Assert.That(xic.XYData.Length, Is.EqualTo(181));

            //Test adding peaks
            var cubicSpline2 = new XicCubicSpline(0.05, 1, 0.1);
            cubicSpline2.SetXicSplineXYData(xic);
            NUnit.Framework.Assert.That(xic.XYData.Min(xy => xy.Item1), Is.EqualTo(0.9).Within(0.0000001));
            NUnit.Framework.Assert.That(xic.XYData.Max(xy => xy.Item1), Is.EqualTo(2.0).Within(0.0000001)); 
            ...
            var linearSpline2 = new XicLinearSpline(1, numberOfPeaksToAdd, 1, true);
            linearSpline2.SetXicSplineXYData(xic1);
            //the xic length after adding the peaks should be 3 + 2*2
            //the first two peaks and the last two peaks should have intensity 0
            NUnit.Framework.Assert.That(xic1.XYData.Length, Is.EqualTo(7));
            for (int i = 0; i < numberOfPeaksToAdd; i++)
            {
                NUnit.Framework.Assert.That(xic1.XYData[i].Item2, Is.EqualTo(0)); 
            }
            ...
            {
                NUnit.Framework.Assert.That(xic1.XYData.First(xy => xy.Item1 == peak.ZeroBasedScanIndex).Item2 == peak.Intensity, Is.True);
            }
        }
        ...
        public static void TestXicSplineExceptionHandling()
        {
            ...
            var ex = NUnit.Framework.Assert.Throws<MzLibException>(() => cubicSpline.GetXicSplineData(rtArray, intensityArray, 1.0, 1.2));
            NUnit.Framework.Assert.That(ex.Message, Is.EqualTo("Input arrays must contain at least 5 points."));
            var intensityArray2 = new float[] { 100, 200, 300, 400, 500 };
            var ex2 = NUnit.Framework.Assert.Throws<MzLibException>(() => cubicSpline.GetXicSplineData(rtArray, intensityArray2, 1.0, 1.2));
            NUnit.Framework.Assert.That(ex2.Message, Is.EqualTo("Input arrays must have the same length."));

            var Bspline = new Bspline(4, 10);
            var ex3 = NUnit.Framework.Assert.Throws<MzLibException>(() => Bspline.GetXicSplineData(rtArray, intensityArray));
            NUnit.Framework.Assert.That(ex3.Message, Is.EqualTo("The number of points in the input array must be greater than the degree of the Bspline."));
        }
        ...
        public static void TestGetAllXics()
        {
            ...
            {
                NUnit.Framework.Assert.That(massXics.Any(x => x.Peaks.Select(p => (IndexedMass)p).First().IsotopicEnvelope.MonoisotopicMass == mass.MonoisotopicMass));
            }
            ...
            {
                NUnit.Framework.Assert.That(xic.Peaks.Count, Is.EqualTo(5));
            }
            //Test with massIndexingEngine
        ...
        public static void TestCutPeak()
        {
            ...
            NUnit.Framework.Assert.That(xic.Peaks.Count, Is.EqualTo(peak1.Count + peak2.Count));
            xic.CutPeak();
            NUnit.Framework.Assert.That(xic.Peaks.Count, Is.EqualTo(peak1.Count));
            NUnit.Framework.Assert.That(xic.ApexRT, Is.EqualTo(xic1.ApexRT));
            NUnit.Framework.Assert.That(xic.StartRT, Is.EqualTo(xic1.StartRT));
            NUnit.Framework.Assert.That(xic.EndRT, Is.EqualTo(xic1.EndRT));

            //if there is only one apex, it should not be cut
            xic1.CutPeak();
            NUnit.Framework.Assert.That(xic1.Peaks.Count, Is.EqualTo(peak1.Count));

            //if the number of peaks is smaller than 5, it should not be cut
            var xic2 = new ExtractedIonChromatogram(peak1.Take(3).ToList());
            xic2.CutPeak();
            NUnit.Framework.Assert.That(xic2.Peaks.Count, Is.EqualTo(3));
            ...
            NUnit.Framework.Assert.That(xic3.Peaks.Count, Is.EqualTo(peak1.Count + peak3.Count));
            xic3.CutPeak();
            NUnit.Framework.Assert.That(xic3.Peaks.Count, Is.EqualTo(peak1.Count + peak3.Count));
        }
        ...
        public static void TestGetAllXicsWithCutPeak()
        {
            ...
            NUnit.Framework.Assert.That(xics.Count(), Is.EqualTo(10)); 
            NUnit.Framework.Assert.That(xics.All(x => x.Peaks.Count == 10), Is.True);
            //The XICs should have an apex at the eighth scan
            NUnit.Framework.Assert.That(xics.All(x => x.ApexScanIndex == 7), Is.True);
            ...
            NUnit.Framework.Assert.That(xicsWithCutPeak.Count(), Is.EqualTo(20));
            NUnit.Framework.Assert.That(xicsWithCutPeak.All(x => x.Peaks.Count == 5), Is.True);
            // The XICs should have an apex at either the third or the eighth scan
            NUnit.Framework.Assert.That(xicsWithCutPeak.All(x => x.ApexScanIndex == 2 || x.ApexScanIndex == 7), Is.True);
        }
        ...
        {
            var peakIndexEngine = PeakIndexingEngine.InitializeIndexingEngine(testScans);
            var ex = NUnit.Framework.Assert.Throws<MzLibException>(() => peakIndexEngine.GetXicByScanIndex(Dist.Masses.First().ToMz(1), zeroBasedStartIndex: 4, new PpmTolerance(20), 1, 10, 1));
            NUnit.Framework.Assert.That(ex.Message, Is.EqualTo("Error: Attempted to access a peak using a charge parameter, but the peaks do not have charge information available."));
        }
        ...
        public static void TestXicOnePeak()
        {
            ...
            NUnit.Framework.Assert.That(xic.Peaks.Count, Is.EqualTo(1));
            NUnit.Framework.Assert.That(xic.ApexRT, Is.EqualTo(peaks.First().RetentionTime));
            NUnit.Framework.Assert.That(xic.ApexScanIndex, Is.EqualTo(peaks.First().ZeroBasedScanIndex));
            NUnit.Framework.Assert.That(xic.StartRT, Is.EqualTo(peaks.First().RetentionTime));
            NUnit.Framework.Assert.That(xic.EndRT, Is.EqualTo(peaks.First().RetentionTime));
            NUnit.Framework.Assert.That(xic.AveragedMassOrMz, Is.EqualTo(peaks.First().M).Within(0.0001));
            NUnit.Framework.Assert.That(xic.ApexPeak.Intensity, Is.EqualTo(peaks.First().Intensity));
        }
        ...
        public static void TestBspline()
        {
            ...
            NUnit.Framework.Assert.That(xic.XYData.MaxBy(p => p.Item2).Item1, Is.EqualTo(xic.ApexRT).Within(0.0001));
            NUnit.Framework.Assert.That(xic.XYData.Max(p => p.Item2), Is.EqualTo(xic.Peaks.Max(p => p.Intensity)).Within(0.0001));
            ...
            NUnit.Framework.Assert.That(xic.XYData.MaxBy(p => p.Item2).Item1, Is.EqualTo(xic.ApexRT).Within(0.001));

            //The start and end RT should always stay the same
            NUnit.Framework.Assert.That(xic.XYData.First().Item1, Is.EqualTo(xic.StartRT).Within(0.0001));
            NUnit.Framework.Assert.That(xic.XYData.Last().Item1, Is.EqualTo(xic.EndRT).Within(0.0001));
        }
    }
}
