...
{
    [TestFixture]
    public class TestDevelopmentTestCases
    {
        [Test]
        public static void TestSinglePeakDeconvolutionTestCase()
        {
            const SampleType sampleType = SampleType.TopDown;
            ...
            var testCase = new SinglePeakDeconvolutionTestCase(sampleType, sampleInformation, pathToDataFile,
                scanNumber, expectedMostAbundantObservedIsotopicMass, expectedIonChargeState, selectedIonMz,
                precursorPpmTolerance);

            Assert.IsNotNull(testCase);
            Assert.That(testCase.SampleType, Is.EqualTo(sampleType));
        ...
        [Test]
        public void TestWholeSpectrumDeconvolutionTestCase()
        {
            const SampleType sampleType = SampleType.TopDown;
            const string sampleInformation = "Direct Injection Cytochrome C, Averaged";
            var pathToDataFile = Path.Combine(TestContext.CurrentContext.TestDirectory, "Deconvolution", "TestData",
            ...
            var testCase = new WholeSpectrumDeconvolutionTestCase(sampleType, sampleInformation, pathToDataFile, scanNumber, 
            ...
            MzLibException exception = Assert.Throws<MzLibException>(() => new WholeSpectrumDeconvolutionTestCase(sampleType, sampleInformation,
