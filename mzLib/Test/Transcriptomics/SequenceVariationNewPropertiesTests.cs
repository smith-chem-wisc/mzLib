...
namespace Test.Transcriptomics
{
    ...
    public class SequenceVariationNewPropertiesTests
    {
        ...
        {
            // NOTE:
            // The SequenceVariation constructors enforce AreValid() (no “no?op” variants:
            // OriginalSequence == VariantSequence and no variant?specific mods). A heterozygous
            // reference representation (ref vs ref) would be a no?op and is therefore rejected.
            // So even with includeReferenceForHeterozygous = true we only get:
            //   Sample 0: HeterozygousAlt (ref copy is invalid -> skipped)
            ...
            Assert.Multiple(() =>
            {
                ...
                // There should be NO HeterozygousRef entry because it is a no-op and invalid.
