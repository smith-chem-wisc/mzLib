...
namespace Test.KoinaTests.RetentionTimePrediction
{
    ...
    ///     unit-testable without a refactor; it is covered by the Koina-category
    ///     tests that hit the real Koina API.
    ///   • Two members are intentionally not unit-tested directly: <c>Dispose</c>
    ///     (empty no-op default; subclasses with unmanaged resources override), and
    ///     the multi-batch throttling delay inside <c>AsyncThrottledPredictor</c>
    ///     (a single <c>await Task.Delay</c> that only fires when an input batch
    ///     spans more than one chunk; pinning it would require timing-aware
    ///     assertions that don't add behavioral value).
    ///
    /// These tests intentionally live OUTSIDE the [Category("Koina")] fixtures so
