...
namespace FlashLFQ
{
    public class FlashLfqEngine
    {
        ...
        private void QuantifyMs2IdentifiedPeptides(SpectraFileInfo fileInfo)
        {
            ...
            Parallel.ForEach(Partitioner.Create(0, ms2IdsForThisFile.Count),
                ...
                (range, loopState) =>
                {
                    ...
                    {
                        chromatographicPeaks[i] = GetChromatographicPeak(ms2IdsForThisFile[i], fileInfo);
                    }
                });
            ...
            DoubleRange peakRegion = null)
        {
            ...
            else
            {
                chromatographicPeak.IsotopicEnvelopes =
                    chromatographicPeak.IsotopicEnvelopes.Where(envelope =>
                        peakRegion.Contains(envelope.IndexedPeak.RetentionTime)).ToList();
            }
        ...
        /// <param name="mass"> Peakfinding mass </param>
        /// <returns></returns>
        public List<IndexedMassSpectralPeak> Peakfind(
            ...
            Tolerance tolerance = null)
        {
            ...
            {
                var peak = _peakIndexingEngine.GetIndexedPeak(mass, t, peakfindingTolerance, charge);
        ...
        /// <summary>
        /// This method searches for identifications where the modified sequence contains the substring "oxidation,"
        /// (e.g. Oxidation, Dioxidation, Trioxidation). Then, it searches for chromatographic peaks
        /// associated with the oxidized identifications. If these peaks overlap (in the RT dimension) with the
        /// chromatographic peak of a less oxidized version of that peptide (e.g., dioxidation -> oxidation),
        /// then the ox mod name string is edited to included the phrase "In-Source".
        ///
        /// Ex: PEPTID[Less Common: Oxidation on D]EK --> PEPTID[Less Common: In-Source Oxidation on D]EK
        /// </summary>
