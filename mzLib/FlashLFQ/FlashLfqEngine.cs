...
namespace FlashLFQ
{
    public class FlashLfqEngine
    {
        ...
        public FlashLfqResults Run()
        {
            ...
            _results = new FlashLfqResults(SpectraFileInfoList, _allIdentifications, FlashParams.MbrQValueThreshold,
                PeptideModifiedSequencesToQuantify, FlashParams.IsoTracker, FlashParams.WriteVerbosePeaks);
            ...
            SpectraFileInfo spectraFile)
        {
            ...
            foreach (IIndexedPeak peak in xic)
            {
                ...
                if (CheckIsotopicEnvelopeCorrelation(massShiftToIsotopePeaks, peak, chargeState, isotopeTolerance, spectraFile, out var pearsonCorr))
                {
                    ...
                    isotopicEnvelopes.Add(FlashParams.WriteVerbosePeaks
                        ? new VerboseIsotopicEnvelope(peak, isotopologuePeaks.ToList(), chargeState,
                            experimentalIsotopeIntensities.Sum(), pearsonCorr, identification.MonoisotopicMass,
                            isotopeTolerance)
                        : new IsotopicEnvelope(peak, chargeState, experimentalIsotopeIntensities.Sum(), pearsonCorr));
                }
            }
