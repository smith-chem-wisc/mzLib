...
public static class MslFormat
{
	...
	/// Version history:
	///   1 — Initial format.
	///   2 — Extended annotation table section added (optional, flagged by
	///       FileFlagHasExtAnnotations). MslFileHeader.Reserved renamed to
	///       ExtAnnotationTableOffset; repurposed to store the absolute byte offset
	///       of the extended annotation table when the flag is set.
	/// </summary>
	...
	// bit 4     : has_ext_annotations   – extended annotation table section is present
	...
	/// <summary>
	/// File-level flag: the extended annotation table section is present (format version 2+).
	/// Set when at least one fragment in the library carries a neutral-loss mass that does not
	/// match any of the five named codes (H2O, NH3, H3PO4, HPO3, PlusH2O).
	/// When this bit is 0 the extended annotation table section is absent from the file and
	/// no seeking is required to decode fragment neutral losses.
	/// When this bit is 1, <see cref="MslFileHeader.ExtAnnotationTableOffset"/> gives the
