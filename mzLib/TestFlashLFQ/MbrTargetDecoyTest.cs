...
namespace Test
{
    ...
    internal class MbrTargetDecoyTest
    {
        ...
        public static void TwoFileMbrTest()
        {
            ...
            var j6Peaks = results.Peaks[j6].Where(p => p.IsMbrPeak & !p.DecoyPeptide & !p.RandomRt).ToList();
            var j6Decoys = results.Peaks[j6].Where(p => p.IsMbrPeak & !p.DecoyPeptide & p.RandomRt).ToList();

            Assert.That(f1r2MbrResults.Count >= 77);

            var allPeaks = results.Peaks.SelectMany(kvp => kvp.Value).Where(p => p.IsMbrPeak).OrderByDescending(p => p.MbrScore).ToList();
            double allPeakCount = 0;
            double decoyPeaks = 0;
            double decoyPeptides = 0;
            double doubleDecoys = 0;
            ...
            foreach(var peak in allPeaks)
            {
                ...
                else if (peak.RandomRt) decoyPeaks++;

                double tempQ = Math.Max(1, decoyPeptides + decoyPeaks - doubleDecoys) / Math.Max(1, allPeakCount);
