...
namespace MassSpectrometry
{
        ...
        where TPeak : IMzPeak
    {
        ...
        public IEnumerable<double> Deconvolute(int maxAssumedChargeState, Tolerance massTol)
        {
            HashSet<TPeak> excluded = new HashSet<TPeak>();
            foreach (var ye in this)
            {
                if (excluded.Contains(ye))
                    continue;

                // This is a monoisotopic peak!

                var bestListOfPeaks = new List<TPeak>();
                int bestChargeState = 1;
                for (int chargeState = 1; chargeState <= maxAssumedChargeState; chargeState++)
                {
                    var mMass = ye.Mz.ToMass(chargeState);
                    var listOfPeaksForThisChargeState = new List<TPeak>();
                    for (int mm = 1; mm <= mms.Length; mm++)
                    {
                        var yeh = mms[mm - 1];
                        var theorMass = mMass + yeh;
                        var closestpeak = GetClosestPeak(theorMass.ToMz(chargeState));
                        if (massTol.Within(closestpeak.Mz.ToMass(chargeState), theorMass) && SatisfiesRatios(mMass, mm, ye, closestpeak))
                        {
                            // Found a match!
                ...
                foreach (var peak in bestListOfPeaks)
                    excluded.Add(peak);
                yield return ye.Mz.ToMass(bestChargeState);
            }
        }
        ...
        private bool SatisfiesRatios(double mMass, int mm, TPeak yeh, TPeak closestpeak)
        {
            ...
            var comparedShouldBe = yeh.Intensity / theMM * theCompared;

            if (closestpeak.Intensity < comparedShouldBe / 5 || closestpeak.Intensity > comparedShouldBe * 5)
                return false;
